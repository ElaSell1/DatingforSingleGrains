...
grain_summary_statistics <- function(datasets, path_for_save = NULL){
  ...
  for (name in names(datasets)){
    ...
    summary_stats[7, name] <- signif(e1071::skewness(dat$De.Gy, na.rm = TRUE), 2)
    ...
    summary_stats[13, name] <- signif(diptest::dip.test(na.omit(as.numeric(dat$De.Gy)))$statistic, 2)
